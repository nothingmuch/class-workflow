...
serialize that:
	...
	sub unpack_instance {
		...
		my %attrs = map { $_->name } $instance->meta->compute_all_applicable_attributes;

		my ( $state, $transition, $prev ) = delete @attrs{qw(state transition prev)};

		$self->_unpack_instance( $instance, $state, $transition, $prev, values %attrs );
	}
	...
	sub _unpack_instance {
		...
		return {
			prev       => $self->_unpack_instance( $prev->get_value($instance), @attrs ),
			state      => $state->get_value($instance)->name, # or some other ID
			transition => $transitions->get_value($instance)->name,
			map { $_->name => $_->get_value($instance) } @attrs,
		}
	}
	...
	sub pack_instance {
		...
		MyInstance->new(
			%$instance,
			prev       => $self->pack_instance( $instance->{prev}, $wf ),
			state      => $wf->get_state( $instance->{state} ),
			transition => $wf->get_transition( $instance->{transition} ),
		);
	}
